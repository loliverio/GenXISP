...
# Phase 1
results_p1 = joinpath("results", "results_p1","power.csv")
power = CSV.read(results_p1, DataFrame, missingstring="NA")
tstart = 3
tend = 400
...
results_b = joinpath("results", "results_p1", "charge.csv")
...
case = joinpath("inputs","inputs_p1","TDR_results","Demand_data.csv")
...
@vlplot(layer=[
    {mark={:area},
     encoding={
        x={:Hour, title="Time Step (hours)", labels="Resource_Type:n", axis={values=0:5-:400}},
